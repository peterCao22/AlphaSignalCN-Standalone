...
def handle_data(context, data):
    ...
    for instrument in list(positions.keys()):
        ...
        # 目标卖出成交日（开盘卖）：buy_exec_day + HOLD_TRADING_DAYS 个交易日
        try:
        ...
        # 今天收盘后下单 => 明天开盘撮合；因此当 next_td == target_sell_td 时触发卖出
        if str(next_td) != str(target_sell_td):
            continue
